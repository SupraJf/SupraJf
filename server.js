...
require('http').createServer((req, res) => res.end('Живой бот')).listen(3000)
...
new Player(client, {
 ...
 client.on('guildCreate', guild =>{
    ...
    const channelId = '988853696563642420'; //put your bot log channel ID here
...
console.log("Subscribe on yt Supra")
  ...
  if (message.channel.name == sender.name) {
    ...
    fetch(
      `https://api.affiliateplus.xyz/api/chatbot?message=${encodeURIComponent(
        message.content
      )}&botname=${client.user.username}&ownername=supra`
    )
      .then(res => res.json())
      .then(data => {
